...
- 左侧以策略卡片展示“通用”“父子分块”和文件类型专属策略；本期只有 Markdown 文件的“MD 优化分块”可用。
...
原文件（本期 Markdown，后续 PDF）
    │
    ▼
分块策略注册表 ChunkStrategyRegistry
    │  按 strategyCode + fileType 选择实现
    ▼
...
持久化预览层 document_chunk（DRAFT）
    │  用户编辑 / 删除 / 下次继续
    ▼
通用上下文增强层 ChunkContextEnricher
    │  最终确认时按 contextPolicy 生成可选 overlap
    ▼
...
- `ChunkContextEnricher`：分块完成后的通用上下文增强器，当前只负责按最终开关生成 overlap；不能被 Markdown parser 或 planner 调用。
...
```text
chunking/
  spi/                 # ChunkPlanningStrategy、通用输入输出模型
  registry/            # descriptor、能力查询、实现注册与校验
  markdown/            # Markdown parser、语义单元、MD 优化 planner
  context/             # 通用 ChunkContextEnricher、Overlap 实现
  preview/             # 预览持久化、编辑、删除
  indexing/            # index_content 构建、向量写入与检索一致性
```

`chunking/markdown` 不能依赖 `chunking/context`；`context` 只依赖已经持久化的通用 `document_chunk`。依赖方向由包结构和单元测试共同约束，防止 overlap 再次渗入 Markdown 解析逻辑。
...
`MarkdownStructureParser` 的输出只包含 Markdown 自己拥有的结构、正文和来源位置。它不知道 overlap 是否开启，也不读取 `overlapTokens`，不复制相邻内容。`MarkdownChunkPlanningStrategy` 同样只决定原文内容所有权和边界。
...
生成预览时在 `file_processing.policy_snapshot` 中保存 `MARKDOWN_OPTIMIZED` 实际生效的分块参数；Overlap 保存到独立 `context_policy`，字段含义见 7.2。
...
Overlap 是文件类型无关的索引/上下文增强能力，由 `ChunkContextEnricher` 在原文 Chunk 已经持久化后处理。Markdown、未来 PDF 或其他分块策略都不负责生成 overlap。
...
| `strategy_code` | varchar | 当前预览选择的分块策略，如 `MARKDOWN_OPTIMIZED` |
| `planner_version` | varchar | 生成当前预览的规划算法版本，如 `markdown-adaptive-v1`、未来的 `pdf-layout-v1` |
| `policy_snapshot` | jsonb | 当前分块策略实际生效的参数与 tokenizer 信息，不包含 overlap |
| `context_policy` | jsonb | 文件类型无关的最终上下文增强参数，当前保存 overlap 开关和长度 |
...
预览生成时写入分块参数。它有三个作用：用户下次打开时恢复当前策略设置；重新生成预览时知道当前边界约束；系统默认值变化后，已经生成的预览不会悄悄改变。参数更新直接覆盖该字段，不产生历史版本。

`context_policy` 与文件解析/分块策略完全分离，在最终确认时写入：
...
单 Chunk 重新向量化与检索上下文继续使用这份设置。未来如果增加其他通用上下文增强能力，也扩展 `context_policy` 和 `ChunkContextEnricher`，不修改 Markdown parser。
...
5. 更新 `strategy_code`、`planner_version`、策略快照、质量统计日志和状态 `CHUNKED`。
...
2. 用户提交最终 `overlapEnabled`；开启时同时提交 `overlapTokens`，默认 40。服务保存到独立 `context_policy`，条件更新文件状态为 `CONFIRMED`，立即禁止编辑。
...
2. 根据最新正文、隐藏的 `overlap_content`、当前 `context_policy` 构建 `index_content`，并再次执行 512 token 硬校验；不再调用 Markdown parser 或 planner。
...
- `422 Unprocessable Entity`：策略未实现、文件类型不匹配、正文为空、文件无 Chunk、源文件 hash 已变化等业务校验失败。
- `404 Not Found`：资源不存在或不属于当前租户/知识库，避免泄露跨租户信息。

## 13. 前端交互约束

页面参考截图的“双栏 + 策略卡片”交互结构，但沿用 StarSeaKnow 现有浅色主题、字体和按钮样式，不复制 Dify 的深色视觉。

建议组件边界：`ChunkingWorkspace` 负责流程状态；`ChunkStrategyPanel/ChunkStrategyCard` 负责能力展示与选择；`MarkdownStrategyConfig` 只负责 MD 参数；`ChunkPreviewPanel/ChunkCard` 负责预览编辑；`ContextConfirmDialog` 独立负责 overlap 和最终确认。这样新增文件类型策略时不需要修改 Chunk 卡片与确认弹窗。
...
- 预览区顶部显示文件名、文件类型和 Chunk 数量。
- 尚未生成时显示空状态，引导用户从左侧生成预览。
- 生成后按顺序展示 Chunk 卡片：不可编辑的完整标题路径、可编辑正文、正文 token 数和当前状态。
...
桌面端左侧设置区约占 38%～42%，右侧预览区占其余空间并独立滚动；窄屏下改为上下排列，策略设置在上、预览在下。

### 13.3 最终确认与 Overlap

- 点击“开始导入/确认向量化”时，在确认弹窗的独立“上下文增强”区域设置 overlap 开关；开启后显示 `overlapTokens`，默认 40。该设置作用于整个文件。
- Overlap 设置不属于任何分块策略卡片，尤其不能出现在“MD 优化分块”参数中。
...
interface ChunkContextEnricher {
    List<EnrichedChunk> enrich(List<DocumentChunk> chunks, ContextPolicy policy);
}
```
...
2. 新增或注册 `PdfChunkPlanningStrategy`，提供独立 `strategyCode`，处理跨页、页眉页脚和版面边界。
3. 将 PDF 页码/区域写入现有 `source_locator`。
4. 在 `planner_version` 写入 PDF 策略版本，在 `policy_snapshot` 写入 PDF 实际参数。
5. 复用现有预览、编辑、删除、`ChunkContextEnricher`、状态机、512 token 校验、向量化、重新向量化和检索后置校验。
...
- 软换行段落不会被误拆成固定问答格式。
- 空章节不生成独立 Chunk。
- Markdown parser 输出不包含 overlap 字段，也不依赖 `context_policy`。
...
- 通用、父子分块卡片始终显示为不可选择；点击或键盘操作均不会选中。
...
7. 预览只展示不可编辑的标题路径和可编辑正文，不展示 overlap。
8. Markdown parser/planner 完全不包含 overlap；Overlap 只由通用 `ChunkContextEnricher` 在最终确认后处理。
9. 最终向量化前可以开启/关闭 overlap；开启时默认 40 tokens，且不截断句子。
10. Embedding 输入与 LLM 上下文完全一致：关闭时为“标题路径 + 正文”，开启时为“标题路径 + overlap + 正文”。
11. 任一最终上下文不超过 512 tokens；预算不足时只能减少 overlap，不能截断或改变用户确认的正文。
