...
public class EvaluationCorpusService {
    ...
    public EvaluationCorpusService(JdbcTemplate jdbc, ObjectMapper json, EvaluationRepository repository,
            ChunkContextEnricher enricher, ChunkRuntimePolicyResolver runtimePolicyResolver,
        ...
        return snapshotTransaction.execute(status -> {
            ...
            if (scope.equals("SELECTED")) {
                ...
                for (var entry : files.entrySet()) {
                    ChunkRuntimePolicy policy = runtimePolicy(entry.getKey(), tenantId, knowledgeId);
                    try {
                        enricher.enrich(entry.getValue(), policy)
                                .forEach(c -> freshTexts.put(c.chunk().getPublicId(), c.indexContent()));
                    } catch (IllegalArgumentException invalidPolicy) {
                        throw EvaluationJson.bad("分块文本与策略快照不一致，请重新生成分块后评测");
                    }
                }
            }
    ...
    private List<Source> loadSources(long tenantId, long knowledgeId) {
        return jdbc.query("""
                ...
                WHERE c.tenant_id=? AND c.knowledge_id=? AND c.status<>1 AND c.chunk_type<>1
            ...
            c.setChunkType(rs.getInt("chunk_type"));
            c.setOverlapEnabled(rs.getBoolean("overlap_enabled")); c.setOverlapLimit(rs.getInt("overlap_limit"));
            c.setOverlapUnit(OverlapUnit.valueOf(rs.getString("overlap_unit")));
    ...
    private ChunkRuntimePolicy runtimePolicy(long fileId, long tenantId, long knowledgeId) {
        List<StoredPolicy> stored = jdbc.query("""
                SELECT strategy_code, policy_snapshot::text, context_policy::text, execution_metadata::text
                FROM file_processing WHERE file_id=? AND tenant_id=? AND knowledge_id=?
                """, (rs, row) -> new StoredPolicy(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4)),
                fileId, tenantId, knowledgeId);
        if (stored.size() != 1 || stored.get(0).strategyCode() == null) {
            throw EvaluationJson.bad("缺少有效的分块策略快照，请重新生成分块后评测");
        }
        StoredPolicy value = stored.get(0);
        try {
            return runtimePolicyResolver.resolve(value.strategyCode(), readMap(value.policySnapshot()),
                    readMap(value.contextPolicy()), readMap(value.executionMetadata()));
    ...
    private Map<String, Object> readMap(String value) throws Exception {
        return value == null ? Map.of() : json.readValue(value, Map.class);
    }

    private record Source(DocumentChunk chunk, String fileName, boolean enabled) {}
    private record StoredPolicy(String strategyCode, String policySnapshot,
                                String contextPolicy, String executionMetadata) {}
}
