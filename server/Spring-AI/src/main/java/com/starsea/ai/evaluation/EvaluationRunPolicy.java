...
final class EvaluationRunPolicy {
    ...
    static void apply(ObjectNode run, ObjectNode calibration) {
        ...
        for (JsonNode result : run.path("modelResults")) {
            ...
            if (calibration != null) {
                ...
                JsonNode threshold = run.path("thresholds").path(model);
                if (!threshold.isNumber() || calibrated == null || !containsThreshold(calibrated.path("calibration"), threshold.asDouble()))
                    missing.add(name + " 使用了校准曲线之外的阈值");
            }
    ...
    private static boolean containsThreshold(JsonNode curve, double threshold) {
        for (JsonNode point : curve) {
                if (point.path("threshold").isNumber()
                    && Math.abs(point.path("threshold").asDouble() - threshold) <= 1e-6) return true;
        }
        return false;
    }
