...
public class EvaluationVectorStore {
    ...
    public List<Score> production(long tenantId, long knowledgeId, String buildKey, double[] query,
        ...
        return transactions.execute(status -> {
            ...
            int candidateLimit = Math.min(topK * 3, 100);
            while (true) {
                ...
                args.add(tenantId); args.add(knowledgeId); args.add(candidateLimit);
                ...
                Set<String> emittedContexts = new java.util.HashSet<>();
                for (Score candidate : candidates) {
                    String contextId = productionContextId(candidate.chunkId(), tenantId, knowledgeId);
                    if (contextId != null && emittedContexts.add(contextId)) permitted.add(candidate);
                }
                permitted.sort(Comparator.comparingDouble(Score::score).reversed().thenComparing(Score::chunkId));
                if (permitted.size() >= topK || candidates.size() < candidateLimit || candidateLimit >= 100) {
                    return List.copyOf(permitted.subList(0,Math.min(topK,permitted.size())));
                }
                candidateLimit = Math.min(100, candidateLimit * 2);
            }
        });
    }

    private String productionContextId(String chunkId, long tenantId, long knowledgeId) {
        try {
            return jdbc.queryForObject("""
                    SELECT CASE WHEN c.chunk_type=2 THEN parent.public_id::text ELSE c.public_id::text END
                    FROM document_chunk c
                    LEFT JOIN document_chunk parent
                      ON parent.id=c.parent_chunk_id
                     AND parent.tenant_id=c.tenant_id
                     AND parent.knowledge_id=c.knowledge_id
                     AND parent.file_id=c.file_id
                    WHERE c.public_id=?::uuid
                      AND c.tenant_id=? AND c.knowledge_id=? AND c.status=2
                      AND c.content IS NOT NULL AND btrim(c.content)<>''
                      AND c.index_content IS NOT NULL AND btrim(c.index_content)<>''
                      AND (c.chunk_type<>2 OR (
                           parent.id IS NOT NULL AND parent.chunk_type=1
                           AND parent.status=2 AND parent.content IS NOT NULL AND btrim(parent.content)<>''
                           AND parent.public_id IS NOT NULL))
                    """, String.class, UUID.fromString(chunkId), tenantId, knowledgeId);
