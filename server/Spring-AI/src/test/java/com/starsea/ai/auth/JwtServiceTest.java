...
class JwtServiceTest {
    ...
    @Test
    void verifyAccess_unrecognizedIssuer_throws40100() {
        String token = Jwts.builder()
                .issuer("unrecognized-issuer")
                .subject("u:1")
                .expiration(Date.from(Instant.now().plusSeconds(60)))
                .signWith(Keys.hmacShaKeyFor(
                        "test-secret-must-be-at-least-32-bytes-long-for-hs256".getBytes(StandardCharsets.UTF_8)),
                        Jwts.SIG.HS256)
                .compact();

        AuthException ex = assertThrows(AuthException.class, () -> svc.verifyAccess(token));
        assertEquals(40100, ex.getCode());
    }
}
