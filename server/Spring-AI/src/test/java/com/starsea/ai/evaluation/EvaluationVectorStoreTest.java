...
class EvaluationVectorStoreTest {
    ...
    @Test void postSortsAnnTiesBeforeSelectingTopKWithoutChangingTheDistanceLimitQuery() {
        ...
        when(jdbc.queryForObject(startsWith("SELECT CASE WHEN c.chunk_type"),eq(String.class),any(Object[].class)))
                .thenAnswer(invocation -> String.valueOf((Object) invocation.getArgument(2)));
    ...
    @Test void productionKeepsOneChildPerParentBeforeApplyingTopK() {
        JdbcTemplate jdbc=mock(JdbcTemplate.class);
        PlatformTransactionManager manager=mock(PlatformTransactionManager.class);
        TransactionStatus transaction=mock(TransactionStatus.class);
        when(manager.getTransaction(any())).thenReturn(transaction);
        when(jdbc.queryForObject(startsWith("SELECT set_config"),eq(String.class),any(Object[].class))).thenReturn("40");
        String first="00000000-0000-0000-0000-000000000011";
        String duplicate="00000000-0000-0000-0000-000000000012";
        String other="00000000-0000-0000-0000-000000000013";
        String parent="00000000-0000-0000-0000-000000000099";
        List<EvaluationVectorStore.Score> candidates=List.of(
                new EvaluationVectorStore.Score(first,.9),
                new EvaluationVectorStore.Score(duplicate,.8),
                new EvaluationVectorStore.Score(other,.7));
        doReturn(candidates).when(jdbc).query(anyString(),
                org.mockito.ArgumentMatchers.<RowMapper<EvaluationVectorStore.Score>>any(),any(Object[].class));
        when(jdbc.queryForObject(startsWith("SELECT CASE WHEN c.chunk_type"),eq(String.class),any(Object[].class)))
                .thenAnswer(invocation -> {
                    String id = UUID.fromString(String.valueOf((Object) invocation.getArgument(2))).toString();
                    if (id.equals(first) || id.equals(duplicate)) return parent;
        ...
        assertEquals(List.of(first,other),selected.stream().map(EvaluationVectorStore.Score::chunkId).toList());
    }
